...
var DesktopManager = GObject.registerClass({
    ...
    acceptDrop(source, xEnd, yEnd) {
        const app = getAppFromSource(source);
        if (app) {
            this._acceptAppIcon(app);
            return
        }
    ...
    _acceptAppIcon(app) {
        const source = Gio.File.new_for_path(app.get_app_info().get_filename());
        const target = Gio.File.new_for_path(DesktopIconsUtil.getDesktopPath() + "/" + source.get_basename());
        source.copy_async(target, 
            ...
            (file, result) => {
                try {
                    ...
                    FileUtils.makeLaunchable(target);
                } catch (e) {
                    log(`Error creating launchable icon: ${e.message}`);
                }
            });    
